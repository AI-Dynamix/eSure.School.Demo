...
export function Schools() {
  ...
  const allowedProvinceNames = useMemo(() => {
      if (isAgency) return ['Thành phố Hồ Chí Minh']
      if (isSSC) return provinceData.filter(p => p.sscPartnerId === 'SSC').map(p => p.name)
  ...
  const filteredSchools = useMemo(() => {
    ...
    if (isAgency) {
      list = list.filter(s => s.province === 'Thành phố Hồ Chí Minh')
        ...
        // Filter by SSC allowable provinces
        const allowed = provinceData.filter(p => p.sscPartnerId === 'SSC').map(p => p.name)
        list = list.filter(s => allowed.includes(s.province))
    }
