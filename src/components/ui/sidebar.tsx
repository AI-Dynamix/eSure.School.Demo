...
function SidebarInset({ className, ...props }: React.ComponentProps<'div'>) {
  ...
  return (
    <div
      ...
      className={cn(
        'group/inset relative flex w-full flex-1 flex-col bg-background',
        'md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ms-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ms-2',
        className
      )}
      {...props}
    >
      {/* Sidebar Toggle Button - visible on sidebar/edge hover */}
      <button
        onClick={toggleSidebar}
        className='absolute -start-3 top-1/2 z-30 hidden h-8 w-6 -translate-y-1/2 items-center justify-center rounded-full border bg-background shadow-sm hover:bg-accent md:flex opacity-0 peer-hover:opacity-100 hover:opacity-100 transition-opacity'
        aria-label='Toggle Sidebar'
      >
        <PanelLeftIcon className={cn('h-4 w-4 transition-transform', state === 'collapsed' && 'rotate-180')} />
      </button>
      {props.children}
    </div>
  )
